...
public class AdvancedWordCountJob {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<WordCount> windowedCountWords =
            splitWords.keyBy(WordCount::getWord).window(TumblingProcessingTimeWindows.of(Time.seconds(5))).sum("count")
                .name("windowed-count");

        DataStream<WordCountResult> windowCoGroup =
            countWords.coGroup(windowedCountWords).where(WordCount::getWord).equalTo(WordCount::getWord)
                .window(TumblingProcessingTimeWindows.of(Time.seconds(10))).apply(
                    new CoGroupFunction<WordCount, WordCount, WordCountResult>() {
                        ...
                        public void coGroup(Iterable<WordCount> first, Iterable<WordCount> second,
                            ...
                            out.collect(
                                new WordCountResult(noWinHead.getWord(), noWinHead.getCount(), windowCount, winWordCount));
                        }
                    }
                ).assignTimestampsAndWatermarks(WatermarkStrategy.noWatermarks()).name("cogroup-window");

        windowCoGroup.addSink(new SinkFunction<WordCountResult>() {
                @Override
