...
public class UniformWordSource extends FromSplittableIteratorFunction<String> {
    ...
    public UniformWordSource() {
        super(new SplittableRateLimitedTextIterator(TimeUnit.MINUTES.toMillis(20),
    ...
    public UniformWordSource(final long totalDuration,
        ...
        super(new SplittableRateLimitedTextIterator(totalDuration, sleepThreshold, burstThreshold));
    }
    ...
    private static class SplittableRateLimitedTextIterator extends SplittableIterator<String> {
        ...
        private SplittableRateLimitedTextIterator(final long totalDuration, final long sleepThreshold, final int burstThreshold) {
        ...
        public Iterator<String>[] split(int nSplits) {
            if (numberOfSplits != nSplits) {
                innerArray = new RateLimitedTextIterator[nSplits];
                for (int i = 0; i < nSplits; i++) {
                    innerArray[i] = RateLimitedTextIterator.unbounded(totalDuration, sleepThreshold, burstThreshold);
                }
